  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="sticky top-0 z-30 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 border-b border-border">
        <div className="mx-auto w-full max-w-sm px-5 h-14 flex items-center">
          <Link
            to="/"
      ...
      <main className="flex-1 flex items-start justify-center px-5 py-8">
        <div className="w-full max-w-sm space-y-7">
          ...
          <div className="grid grid-cols-2 gap-2">
            {opcoes.map((opcao) => (
              <button
                ...
                className={`rounded-xl border px-2 py-3.5 text-sm font-medium transition-all ${
          ...
          {tipoCadastro === "motorista" && (
            <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3">
              ...
              <p className="text-[11px] text-muted-foreground leading-snug mt-1">
                ...
                corrida ou agendar um serviço, peça ao profissional o link correto.
              </p>
            </div>
          )}
          ...
          <form onSubmit={handleCadastro} className="space-y-4">
            {precisaSlug && (
              <div className="space-y-2">
                ...
                {tipoCadastro === "afiliado" && (
                  <p className="text-xs text-muted-foreground">
                    Você será cadastrado como afiliado neste grupo.
                  </p>
                )}
                {tipoCadastro === "motorista" && (
                  <p className="text-xs text-muted-foreground">
                    Você enviará uma solicitação para entrar no grupo. O dono precisa aprovar antes de receber corridas.
                  </p>
                )}
              </div>
            )}
            ...
            <Button type="submit" size="lg" className="w-full h-12 rounded-xl text-base font-semibold" disabled={carregando}>
