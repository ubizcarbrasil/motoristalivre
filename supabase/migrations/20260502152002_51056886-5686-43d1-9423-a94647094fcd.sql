-- 1. ride_id passa a ser opcional (comissões de serviço não têm ride)
ALTER TABLE public.commissions ALTER COLUMN ride_id DROP NOT NULL;

-- 2. Nova coluna booking_id para comissões de serviço
...
-- 3. Reescreve o motor para usar booking_id
...
BEGIN
  -- (1) Idempotência: por booking_id
  IF EXISTS (
    SELECT 1 FROM public.commissions
    WHERE booking_id = _booking_id AND status = 'processed'
  ...
  THEN
    ...
    IF _attendant_wallet.id IS NOT NULL AND _origin_wallet.id IS NOT NULL AND _comissao_cobertura > 0 THEN
      ...
      INSERT INTO public.commissions (
        booking_id, tenant_id, commission_type, amount,
